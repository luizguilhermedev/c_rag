...
def process_batch(state: DocumentProcessorState) -> DocumentProcessorState:
    ...
    try:
        ...
        try:
            ...
            if start_idx >= 0 and end_idx > start_idx:
                try:
                    ...
                    # Fallback: treat as book content
                    batch_result = {
                        "book_content": batch,
                        "summary_content": ""
                    }
        ...
        batch_file = os.path.join(batch_dir, f"batch_{state.current_batch_idx + 1:03d}.json")
        with open(batch_file, "w", encoding="utf-8") as f:
            json.dump(batch_result, f, ensure_ascii=False, indent=2)
            print(f"Saved batch {state.current_batch_idx + 1}/{len(state.batches)} to {batch_file}")
...
def save_results(state: DocumentProcessorState) -> DocumentProcessorState:
    ...
    print(f"✅ Final JSON saved to {json_path} (ready for chunking)")
...
class DocumentPreProcessorAgent:
    def __init__(self, model_name: Optional[str] = None, output_dir: str = "processed_documents"):
    ...
    def preprocess_document(self, document_text: str, document_name: str = "document") -> Tuple[List[Chunk], List[Chunk], str]:
        """
        ...
        book_chunks = [Chunk(
            ...
            metadata={"source_type": "book_content", "document_name": document_name}
        )]
...
if __name__ == "__main__":
    ...
    print(f"\nProcessing complete!")
    print(f"Book content length: {len(book_chunks[0].text) if book_chunks else 0} characters")
    print(f"Summary content length: {len(summary_chunks[0].text) if summary_chunks else 0} characters")
    print(f"JSON file saved to: {json_path}")
    print(f"This JSON file can now be used for chunking.")
